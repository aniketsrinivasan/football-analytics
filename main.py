from utils import read_video, save_video, convert_avi_to_mp4
...
def main():
    ...
    if DEBUGGING:
        print(f"Getting object tracks...")
    tracks = tracker.get_entity_tracks(video_frames,
                                       read_from_stub=True,
                                       stub_path=ROOT_DIR+"/stubs/track_stubs.pkl")
    if DEBUGGING:
        print(f"Successfully created object tracks.")

    # Get positions of objects:
    tracker.add_position_to_tracks(tracks)
    if DEBUGGING:
        print(f"Added positions to tracks.")

    # # Camera movement estimation:
    # if DEBUGGING:
    #     print(f"Estimating camera movement...")
    ...
    # if DEBUGGING:
    #     print(f"Successfully estimated camera movements.")
    #
    # # Adding adjusted tracks of objects, based on camera movement:
    # camera_movement_estimator.add_adjust_positions_to_tracks(tracks, camera_movement_per_frame)
    # if DEBUGGING:
    #     print(f"Added adjustment positions to tracks.")
    #
    # # # Running the view transformer (for perspective transformation):
    # # view_transformer = ViewTransformer()
    # # view_transformer.add_transformed_position_to_tracks(tracks)
    # if DEBUGGING:
    #     print(f"Perspective transformation successful.")
    #
    # Interpolate ball positions:
    tracks["ball"] = tracker.interpolate_ball_positions(tracks["ball"])
    if DEBUGGING:
        print(f"Interpolated ball positions.")
